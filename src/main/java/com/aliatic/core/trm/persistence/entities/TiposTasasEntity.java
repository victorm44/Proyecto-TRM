...
@Entity
@Table(name="TRM_TIPOSTASAS")
public class TiposTasasEntity extends BaseEntity {
	
	@Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "IDTPTASA")
    private Long idTipoTasa;
	
    @Size(max = 60)
    @Column(length = 60, name = "CDTPTASA")
	private String codigoTipoTasa;
	
    
    @Size(max = 100)
    @Column(length = 100, name = "DSTPTASA")
